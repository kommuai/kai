...
from shadou.content.copy import get_chat_copy, reload_chat_copy
from shadou.lib.lang import resolve_lang
from shadou.lib.media_handler import init_media_log
...
from shadou.services.turn_ingest import ingest_user_turn
from shadou.settings import get_settings

log = logging.getLogger("shadou")
...
class ShadouService:
    ...
    def finalize_reply(self, conversation_id, answer: str, lang: str, *, suppress: bool = False) -> str:
        ...
        from shadou.core.outbound_delivery import prepare_outbound_reply
    ...
    def _handle_learning_command(
        ...
        from shadou.lib.learning_events import fetch_pending_events, set_event_status
    ...
    def _learning_advance(
        ...
        from shadou.lib.learning_events import fetch_pending_events
    ...
    def _handle_admin_answer(
        ...
        try:
            from shadou.lib.learning_events import fetch_pending_events, set_event_status
            from shadou.support_runtime.admin_learn import generate_learning_proposal
    ...
    def pre_router(self, data: dict) -> dict | None:
        ...
        log.info("[Shadou] IN conv=%s type=text text=%s", conversation_id, text)
        ...
        if sess.get("frozen"):
            ...
            if auto_unfreeze_stale_handoff(conversation_id, idle_hours=idle_h):
                log.info(
                    "[Shadou] auto-unfreeze after %sh handoff idle conv=%s",
                    idle_h,
                    conversation_id,
                )
                sess = get_session(conversation_id)
