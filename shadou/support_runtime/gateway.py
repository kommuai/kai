...
from shadou.support_runtime.faq_grounding import apply_grounding_footnote_if_needed, is_answer_faq_grounded
from shadou.support_runtime.agent_loop import _looks_like_chitchat
from shadou.support_runtime.metrics import record_turn_metrics
from shadou.support_runtime.models import RuntimeResult
from shadou.support_runtime.hitl import maybe_record_hitl_ticket
...
def run_support_turn(
    *,
    ...
    from shadou.lib.llm_usage_record import resolve_usage_tenant_slug, set_usage_context
    ...
    if use_pre_router:
        pre = shadou_service.pre_router({"content": text, "phone_number": user_id or "unknown"})
        if pre is not None:
    ...
    if result.decision == "escalate_human":
        ...
        freeze(user_id, True)
        msg = shadou_service.finalize_reply(user_id, result.answer, lang, suppress=True)
    ...
    msg = shadou_service.finalize_reply(user_id, answer_text, lang, suppress=suppress_footer)
    record_turn_metrics(result)
    return SupportTurnOutcome(kind="reply", message=msg, runtime=result, next_state="bot")
