# Shadou WhatsApp Bridge (Baileys)

Sidecar for **QR linking** (Studio onboarding) and **multi-tenant message worker** (inbound → Shadou → WhatsApp reply).
...
npm install
export SHADOU_TENANTS_ROOT=/home/ting/workspace   # parent of shadou-tenant-* dirs
export SHADOU_REPO=/home/ting/workspace/shadou
npm start
```
...
| Worker manager | Scans `SHADOU_TENANTS_ROOT` every 15s for `whatsapp_baileys` tenants |
...
| Variable | Default |
|----------|---------|
| `SHADOU_TENANTS_ROOT` | `~/workspace` |
| `SHADOU_REPO` | `studio/whatsapp-bridge/../..` |
| `SHADOU_PYTHON` | `python3` |
...
WhatsApp message → download voice note (if any) → `shadou_inbound.py` (`SHADOU_HOME` = tenant workspace) → STT enrichment → `support_runtime` → reply sent via Baileys. Sessions appear in Studio **Inbox** (`data/sessions.db`).
...
The WhatsApp bridge starts a warm STT sidecar (`python -m shadou.media.stt_server`, port `18792`) so the model loads once, not per message.

| Variable | Default | Purpose |
|----------|---------|---------|
| `SHADOU_STT_PROVIDER` | — | Override workspace: `faster_whisper` (free local) or `openai_whisper` (paid API) |
| `SHADOU_STT_MODEL` | `small` | Local: `tiny`/`base`/`small`/`medium`. API: `whisper-1` |
| `SHADOU_STT_DEVICE` | `auto` | `cuda` or `cpu` for local Whisper |
| `SHADOU_STT_COMPUTE_TYPE` | auto | `float16` (GPU) or `int8` (CPU) |
| `SHADOU_STT_SERVER_PORT` | `18792` | Local STT sidecar port |
...
systemctl --user enable --now shadou-whatsapp-bridge.service
```
