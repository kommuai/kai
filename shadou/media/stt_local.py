...
def _device_and_compute() -> tuple[str, str]:
    device = (os.getenv("SHADOU_STT_DEVICE") or "auto").strip().lower()
    compute = (os.getenv("SHADOU_STT_COMPUTE_TYPE") or "").strip().lower()
    if device == "auto":
...
def _load_model() -> Any:
    ...
    model_name = (os.getenv("SHADOU_STT_MODEL") or cfg.stt_model or "small").strip()
