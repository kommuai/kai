#!/usr/bin/env python3
"""CLI to exercise Shadou HTTP APIs (message, query, search, admin).

Examples:
  # Single message (WhatsApp/n8n shape)
  python3 tools/shadou_api_cli.py message "What cars are supported?"
  python3 tools/shadou_api_cli.py message "Corolla Cross install" --phone +60123456789

  # Multi-turn REPL (same session / phone_number)
  python3 tools/shadou_api_cli.py chat --phone test-cli-user

  # Machine-agent endpoints (needs x-api-key)
  python3 tools/shadou_api_cli.py query "What is KommuAssist?"
  python3 tools/shadou_api_cli.py search "installation appointment"

  # Admin (set ADMIN_TOKEN in env)
  python3 tools/shadou_api_cli.py admin refresh-sop
  python3 tools/shadou_api_cli.py admin reset-memory --phone +60123456789

Env:
  SHADOU_API_BASE_URL   default http://127.0.0.1:6090
  SHADOU_API_KEY        default internal-key (for /v2/agent/query|search)
...
DEFAULT_BASE = os.getenv("SHADOU_API_BASE_URL", "http://127.0.0.1:6090").rstrip("/")
DEFAULT_API_KEY = os.getenv("SHADOU_API_KEY", "internal-key")
DEFAULT_PHONE = os.getenv("SHADOU_API_PHONE", "cli-test-user")
...
def cmd_chat(args: argparse.Namespace) -> int:
    print(f"Shadou API chat — phone={args.phone} base={args.base}")
...
def build_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="Shadou HTTP API test CLI")
