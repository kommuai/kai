...
def require_strong_admin_token_enabled() -> bool:
    if os.getenv("SHADOU_REQUIRE_STRONG_ADMIN_TOKEN", "").strip().lower() in {"1", "true", "yes", "on"}:
        return True
    return os.getenv("SHADOU_STRICT_STARTUP", "").strip().lower() in {"1", "true", "yes", "on"}


def assert_admin_token_acceptable_for_boot() -> None:
    if require_strong_admin_token_enabled() and admin_token_is_weak():
        raise RuntimeError(
            "ADMIN_TOKEN is missing or weak; set a long random token or disable SHADOU_STRICT_STARTUP / "
            "SHADOU_REQUIRE_STRONG_ADMIN_TOKEN"
        )
