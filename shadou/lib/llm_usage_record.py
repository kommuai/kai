"""Persist LLM token usage for Shadou Studio dashboard (shared SQLite admin DB)."""
...
log = logging.getLogger("shadou.llm_usage")
...
def resolve_usage_tenant_slug(explicit: str | None = None) -> str | None:
    ...
    home = (os.getenv("SHADOU_HOME") or "").strip()
    if home:
        name = Path(home).name
        if name.startswith("shadou-tenant-"):
            return name.removeprefix("shadou-tenant-")
    if explicit and str(explicit).strip():
        return str(explicit).strip()
    try:
        from shadou.workspace.manifest import load_manifest
...
def _admin_db_path() -> Path | None:
    raw = (os.getenv("SHADOU_ADMIN_DB_DIR") or "").strip()
    if not raw:
        return None
...
def record_openai_usage(
    *,
    ...
    """Parse usage, compute USD, append row when SHADOU_ADMIN_DB_DIR is set."""
