"""Eval comparison tool for Kai — compare baseline vs candidate eval results.

Usage:
    python -m kai.tools.eval_compare --baseline results_v1.json --candidate results_v2.json

Exit codes:
    0 — no critical regression
    1 — accuracy_critical regressed, or any configured critical gate failed

Inputs are JSON files produced by ``kai.tools.eval_run`` (with ``--output PATH``).
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Kai eval comparison")
