# Shadou Studio — agent context

**Location:** `studio/` in the Shadou monorepo (`workspace/shadou/studio/`). The standalone `workspace/shadou-admin-ui` tree was removed after this merge.

## Session log

### 2026-05-29 — Chatwoot removed entirely
- Deleted `shadou_chatwoot_studio.py`, inbox Chatwoot routes, ConversationPage Chatwoot panel, engine `shadou/integrations/chatwoot/`.
...
- **Change:** `ConversationPage.tsx` — removed Memory card and Chatwoot card when not configured or no linked conversation (no empty “not configured” panel). Tags: chips + add form only after Tags icon (mobile sheet or desktop expand). Chatwoot on mobile: bottom sheet via message icon when `SHADOU_CHATWOOT_*` configured; desktop Chatwoot column only when a linked `conversation_id` exists. Tighter header, safe-area on composer, `min-h-0` scroll chain. `InboxPage` detail column `min-h-0`.
...
- **Backend:** `shadou_chatwoot_studio.py` worker actions: `get_meta`, `set_status`, `private_note`, `set_labels`, `human_handover`, `resume_bot`, `account_labels` (empty list when Chatwoot not configured). `inbox_router.py` routes for meta, status, private note, labels, handover, resume; `ConversationDetailOut.chatwoot_conversation_id` from session JSON.
...
- **Runtime:** `shadou/integrations/chatwoot/client.py` — `set_conversation_status`, `create_private_note`, label GET/POST, `list_account_labels`, etc.
...
- **Studio reply:** `POST /tenants/{id}/inbox/conversations/{user_id}/reply` runs `backend/shadou_reply.py` with tenant `SHADOU_HOME` — appends `agent` role to sessions.db, optional Chatwoot outbound when `chatwoot_conversation_id` is on the session.
- **Shadou runtime:** `shadou_service.pre_router` persists `chatwoot_conversation_id` from webhook payloads into session JSON for later Studio delivery.
...
### 2026-05-27 — Inbox, Conversations & Contacts (Shadou Studio)
...
- Inserted SQLite `tenants` row: `slug=kommu`, `workspace_home=/home/ting/workspace/shadou-tenant-kommu`, owner = user with email `yuanting@kommu.ai`.
