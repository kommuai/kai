...
log = logging.getLogger("shadou.providers")
...
class DeepSeekProvider:
        ...
        if not self.cfg.api_key:
            if not DeepSeekProvider._missing_key_warned:
                log.error(
                    ...
                    "Set SHADOU_LLM_API_KEY or DEEPSEEK_API_KEY (empty values shadow the alias). "
    ...
    def embed(self, texts: list[str]) -> list[list[float]]:
        ...
        if self.cfg.api_key:
            try:
                model = self.cfg.embedding_model or get_settings().shadou_embed_model
    ...
    def chat_messages(self, messages: list[dict], *, temperature: float = 0.2, max_tokens: int = 1200) -> str:
        if not self.cfg.api_key:
            if not OpenAICompatibleProvider._missing_key_warned:
                log.error(
                    "LLM call skipped: no API key configured. "
                    "Set SHADOU_LLM_API_KEY (empty value shadows the alias). "
                    "Replies will fall back to clarify text until this is fixed."
                )
...
def build_provider() -> ChatProvider:
    s = get_settings()
    provider_name = s.shadou_llm_provider.strip().lower()
    if provider_name == "deepseek":
        cfg = ProviderConfig(
            provider="deepseek",
            model=s.shadou_llm_model,
            base_url=s.shadou_llm_base_url,
            api_key=s.shadou_llm_api_key,
            embedding_model=s.shadou_embed_model,
        )
        return DeepSeekProvider(cfg)

    cfg = ProviderConfig(
        provider=provider_name,
        model=s.shadou_llm_model or "gpt-4o-mini",
        base_url=s.shadou_llm_base_url,
        api_key=s.shadou_llm_api_key,
        embedding_model=s.shadou_embed_model,
    )
