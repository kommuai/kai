...
from shadou.support_runtime.tools.site_search import match_vehicle_catalog, support_site_corpus
...
class ToolHandlers:
    ...
    def search_session_memory(self, query: str, limit: int = 5) -> dict[str, Any]:
        from shadou.lib.session_search import search_user_messages
    ...
    def search_github_repo(self, query: str, branch: str = "") -> dict[str, Any]:
        from shadou.support_runtime.tech_backlog import github_repo_agentic_search

        p = self._params("search_github_repo")
        repo = str(p.get("repo") or os.getenv("SHADOU_GITHUB_REPO", "")).strip().strip("/")
        br = (branch or str(p.get("branch") or os.getenv("SHADOU_GITHUB_BRANCH", ""))).strip()
    ...
    def read_github_file(self, path: str, branch: str = "") -> dict[str, Any]:
        ...
        repo = str(p.get("repo") or os.getenv("SHADOU_GITHUB_REPO", "")).strip().strip("/")
        br = (branch or str(p.get("branch") or os.getenv("SHADOU_GITHUB_BRANCH", "main"))).strip() or "main"
        ...
        token = (os.getenv("SHADOU_GITHUB_TOKEN") or "").strip()
        if token:
    ...
    def lookup_sheet_record(self, record_id: str = "", dongle_id: str = "") -> dict[str, Any]:
        from shadou.lib.google_sheets import warranty_lookup_by_dongle, warranty_text_from_row
    ...
    def lookup_sheet_backlog(self, issue_summary: str) -> dict[str, Any]:
        from shadou.support_runtime.tech_backlog import find_similar_active_issue
    ...
    def log_sheet_backlog(
        ...
        **_ignored: Any,
        ...
        from shadou.lib.deepseek_client import chat_completion as deepseek_chat_completion
        from shadou.support_runtime.tech_backlog import append_backlog_issue, summarize_issue
