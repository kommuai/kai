...
from kai.engine.startup import should_compile_at_startup
from kai.settings import get_settings


class StartupCompilePolicyTests(unittest.TestCase):
    def test_compile_off(self):
        with mock.patch.dict(os.environ, {"KAI_STARTUP_COMPILE": "0"}, clear=False):
    ...
    def test_compile_auto_when_chunks_missing(self):
        ...
        try:
            if existed:
                chunks.unlink()
            with mock.patch.dict(os.environ, {"KAI_STARTUP_COMPILE": "auto"}, clear=False):
