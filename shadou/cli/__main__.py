#!/usr/bin/env python3
"""Usage: python -m shadou.cli <workspace|pack|doctor|compile|port-check|paths>"""
...
from shadou.cli.pack import export_pack, install_pack, scaffold_workspace
from shadou.settings import get_settings, reload_settings
from shadou.settings.loader import BASE_DIR
from shadou.settings.paths import resolve_shadou_home, using_deprecated_agent_workspace
from shadou.support_runtime.compiler import compile_canonical_knowledge
from shadou.workspace.validate import ValidationIssue, validate_workspace, workspace_is_healthy
...
def cmd_doctor(args: argparse.Namespace) -> int:
    if using_deprecated_agent_workspace():
        print("[WARN] AGENT_WORKSPACE is deprecated; use SHADOU_HOME instead.")
...
def cmd_port_check(_args: argparse.Namespace) -> int:
    from shadou.engine.features import get_workspace_features
    ...
    print(f"SHADOU_HOME={settings.shadou_home}")
...
def cmd_paths(_args: argparse.Namespace) -> int:
    from shadou.workspace.manifest import load_workspace_manifest, workspace_yaml_path

    manifest = load_workspace_manifest()
    home = get_settings().shadou_home
    print(f"SHADOU_HOME={home}")
...
def cmd_workspace_init(args: argparse.Namespace) -> int:
    home = Path(args.home or resolve_shadou_home()).expanduser().resolve()
    try:
        ...
        return 1
    reload_settings()
    print(f"Initialized SHADOU_HOME at {target} ({copied} files from template).")
    print("Next:")
    print("  shadou pack install <tenant-pack>   # optional tenant content")
    print("  shadou doctor")
    return 0
...
def cmd_pack_export(args: argparse.Namespace) -> int:
    home = Path(args.home).expanduser() if args.home else get_settings().shadou_home
    out = export_pack(
        args.output or f"shadou-pack-{home.name}.tar.gz",
        home=home,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="shadou", description="Shadou agent harness CLI")
    sub = parser.add_subparsers(dest="command", required=True)

    p_ws = sub.add_parser("workspace", help="Manage SHADOU_HOME workspace")
    ws_sub = p_ws.add_subparsers(dest="workspace_cmd", required=True)
    p_ws_init = ws_sub.add_parser("init", help="Scaffold ~/.shadou from generic template")
    p_ws_init.add_argument("--home", default="", help="SHADOU_HOME path (default ~/.shadou or SHADOU_HOME env)")
    ...
    p_pack_in = pack_sub.add_parser("install", help="Install tenant pack into SHADOU_HOME")
    ...
    p_pack_ex = pack_sub.add_parser("export", help="Export SHADOU_HOME as tar.gz")
    ...
    p_init = sub.add_parser("init", help="[deprecated] Use: shadou workspace init")
    p_init.add_argument("--workspace", default=str(resolve_shadou_home()), help="Target SHADOU_HOME")
    p_init.add_argument("--force", action="store_true")
    p_init.set_defaults(func=cmd_init)

    for name in ("doctor", "validate"):
        p = sub.add_parser(name, help="Validate SHADOU_HOME layout, FAQ, tools")
    ...
    p_paths = sub.add_parser("paths", help="Print resolved SHADOU_HOME paths")
    ...
    p_export = sub.add_parser("export-pack", help="[deprecated] Use: shadou pack export")
    ...
    p_export.set_defaults(
        func=lambda ns: cmd_pack_export(
            argparse.Namespace(
                output=ns.output,
                home=ns.workspace or str(get_settings().shadou_home),
