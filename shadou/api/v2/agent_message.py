...
from shadou.core.policy.settings import RouteMode, get_route_mode
from shadou.content.channels import get_channel_config
from shadou.engine.metrics import inc as metrics_inc
...
from shadou.support_runtime.gateway import run_support_turn

log = logging.getLogger("shadou.v2")
router = APIRouter()
...
def _process_agent_message_data(data: dict, *, x_admin_token: str | None = None) -> dict:
    ...
    if msg_type in ("voice", "audio") and media_path:
        from shadou.media.enrich import enrich_inbound_media
    ...
    if (
        ...
        try:
            from shadou.lib.learning_events import record_event
            record_event(
                user_id=user_id,
                user_text=text,
    ...
    debug_env = bool(s.shadou_route_agent_debug_enabled)
...
async def admin_reset_memory(request: Request, x_admin_token: str | None = Header(default=None)):
    ...
    msg = shadou_service.admin_reset_memory(user_id)
