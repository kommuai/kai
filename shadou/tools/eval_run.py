"""Tenant-agnostic eval harness for Shadou support agents.

Usage:
    python -m shadou.tools.eval_run --eval-file PATH [--open-book] [--gates-file PATH]
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if line:
            try:
                ...
                pass
    return expand_eval_items(items)


def expand_eval_items(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Expand variant_group rows into multiple eval rows sharing metadata."""
    out: list[dict[str, Any]] = []
    groups: dict[str, list[dict[str, Any]]] = {}
    for item in items:
        vg = item.get("variant_group")
        if vg:
            groups.setdefault(str(vg), []).append(dict(item))
        else:
...
def run_eval(
    ...
    *,
    ...
    from shadou.support_runtime.faq_grounding import is_answer_faq_grounded
    ...
    for item in eval_items:
        ...
        tool_names = [getattr(e, "tool", "") or "" for e in (result.evidence_ledger or [])]
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Shadou eval harness")
    ...
    if args.workspace_yaml:
        ...
        try:
            from shadou.workspace.manifest import workspace_yaml_path as _wsp
