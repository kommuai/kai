...
def _stt_server_url() -> str:
    return (os.getenv("SHADOU_STT_SERVER_URL") or "http://127.0.0.1:18792").rstrip("/")


def _stt_credentials() -> tuple[str, str, str]:
    api_key = (
        os.getenv("SHADOU_STT_API_KEY")
        or os.getenv("OPENAI_API_KEY")
        or ""
    ).strip()
    base_url = (os.getenv("SHADOU_STT_BASE_URL") or "https://api.openai.com/v1").rstrip("/")
    cfg = get_media_config()
    model = (os.getenv("SHADOU_STT_MODEL") or cfg.stt_model or "whisper-1").strip()
