...
from shadou.settings.paths import using_deprecated_agent_workspace
from shadou.support_runtime.compiler import compile_canonical_knowledge
from shadou.support_runtime.tools.catalog import builtin_catalog, resolve_builtin_id
from shadou.workspace.manifest import load_workspace_data, load_workspace_manifest, workspace_yaml_path
from shadou.workspace.tools_config import load_tools_config
...
def validate_workspace(*, compile_kb: bool = True, ping_llm: bool = False) -> list[ValidationIssue]:
    ...
    if using_deprecated_agent_workspace():
        issues.append(
            ValidationIssue(
                ...
                "AGENT_WORKSPACE is deprecated; set SHADOU_HOME instead",
            )
        )
    ...
    for entry in tools_cfg.enabled_entries():
        if entry.plugin:
            from shadou.tools_plugins.runner import resolve_plugin_script
            ...
            if script:
                ...
                from shadou.tools_plugins.contract import validate_plugin_file
    ...
    api_key = (settings.shadou_llm_api_key or settings.deepseek_api_key or "").strip()
    if not api_key:
        issues.append(ValidationIssue("warn", "llm_key_missing", "No LLM API key (SHADOU_LLM_API_KEY / DEEPSEEK_API_KEY)"))
    elif ping_llm:
        try:
