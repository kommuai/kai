"""Resolve KAI_HOME and standard paths under the tenant workspace."""
...
def default_kai_home() -> Path:
    return Path.home() / ".kai"


def resolve_kai_home(*, base_dir: Path | None = None) -> Path:
    """Workspace root: KAI_HOME, else deprecated AGENT_WORKSPACE, else ~/.kai."""
    base = base_dir or BASE_DIR
    explicit = os.getenv("KAI_HOME", "").strip()
    if explicit:
    ...
    if legacy:
        ...
        return path if path.is_absolute() else base / path
    return default_kai_home()


def resolve_env_file(home: Path) -> Path:
    explicit = os.getenv("KAI_ENV_FILE", "").strip()
    if explicit:
        path = Path(explicit).expanduser()
...
def resolve_sop_sync_state_path(home: Path) -> Path:
    explicit = os.getenv("KAI_SOP_SYNC_STATE_PATH", "").strip()
    if explicit:
        path = Path(explicit).expanduser()
        return path if path.is_absolute() else BASE_DIR / path
...
class KaiPaths:
    """Canonical layout under KAI_HOME."""
...
def using_deprecated_agent_workspace() -> bool:
    return bool(os.getenv("AGENT_WORKSPACE", "").strip()) and not os.getenv("KAI_HOME", "").strip()
