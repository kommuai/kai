"""Tenant pack install/export into SHADOU_HOME."""
...
from shadou.settings.paths import ShadouPaths
...
def install_pack(src: str | Path, *, force: bool = False) -> tuple[int, list[str]]:
    """Copy tenant pack files into SHADOU_HOME. Returns (copied_count, messages)."""
    home = get_settings().shadou_home
    paths = ShadouPaths(home)
    ...
    try:
        if str(src).startswith(("http://", "https://")):
            tmp_dir = tempfile.TemporaryDirectory(prefix="shadou-pack-")
...
def export_pack(
    ...
    *,
    ...
    """Tar.gz SHADOU_HOME (or subset) for portability."""
    src = home or get_settings().shadou_home
...
def scaffold_workspace(home: Path | None = None, *, force: bool = False) -> tuple[int, Path]:
    """Initialize SHADOU_HOME from engine generic template."""
    target = home or get_settings().shadou_home
    template = _template_root()
    if not template.is_dir():
        raise FileNotFoundError(f"Template missing: {template}")

    if target.exists() and any(target.iterdir()) and not force:
        raise FileExistsError(f"SHADOU_HOME is not empty: {target}")

    ShadouPaths(target).ensure_runtime_dirs()
    copied = 0
