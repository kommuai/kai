...
from shadou.support_runtime.models import RetrievalItem
from shadou.support_runtime.providers import ChatProvider
...
_log = logging.getLogger("shadou.retrieval")


def _chunks_path() -> Path:
    return get_settings().shadou_home / "compiled" / "kb_chunks.jsonl"
...
class HybridRetriever:
    ...
    def __init__(self, provider: ChatProvider | None = None) -> None:
        ...
        s = get_settings()
        self.collection = s.shadou_qdrant_collection
        self.use_qdrant = s.shadou_qdrant_enabled
        self.client = None
    ...
    def load(self) -> None:
        ...
        if self.use_qdrant and not QdrantClient:
            _log.warning(
                "SHADOU_QDRANT_ENABLED is set but qdrant-client is not installed; using local retrieval only"
            )
        if self.use_qdrant and QdrantClient:
            try:
                s = get_settings()
                self.client = QdrantClient(
                    url=s.shadou_qdrant_url,
                    api_key=s.shadou_qdrant_api_key or None,
                )
            except Exception:
                self.client = None
...
class SimpleReranker:
    ...
    def __init__(self, provider: ChatProvider | None = None) -> None:
        self.provider = provider
        self.backend = get_settings().shadou_reranker_backend.strip().lower()
