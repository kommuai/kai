...
# Repo .env first; SHADOU_HOME/.env loaded after home resolution in load_settings()
...
def _env_nonempty(*names: str, default: str = "") -> str:
    ...
    Empty assignments (e.g. ``SHADOU_LLM_API_KEY=``) are treated as unset so a
    later alias (e.g. ``DEEPSEEK_API_KEY``) can take effect. Previously an
    explicit ``SHADOU_LLM_API_KEY=`` in a ``.env`` template silently shadowed the
...
class Settings:
    ...
    shadou_llm_provider: str = "deepseek"
    shadou_llm_model: str = ""
    shadou_llm_base_url: str = ""
    shadou_llm_api_key: str = ""
    shadou_embed_model: str = "text-embedding-3-small"

    # Retrieval
    shadou_qdrant_enabled: bool = False
    shadou_qdrant_url: str = "http://127.0.0.1:6333"
    shadou_qdrant_api_key: str = ""
    shadou_qdrant_collection: str = "shadou_support"
    shadou_reranker_backend: str = "provider"
    shadou_guardrails_enabled: bool = False

    # SOP
    shadou_sop_writeback_enabled: bool = False
    shadou_sop_merge_sync_enabled: bool = False
    shadou_sop_merge_sync_hour: int = 8
    shadou_sop_merge_sync_minute: int = 0
    ...
    shadou_home: Path = field(default_factory=lambda: Path.home() / ".shadou")
    agent_workspace: Path = field(default_factory=lambda: Path.home() / ".shadou")
    master_faq_path: Path = field(default_factory=lambda: Path.home() / ".shadou" / "knowledge" / "master_faq.md")
    faq_learn_queue_dir: Path = field(default_factory=lambda: Path.home() / ".shadou" / "knowledge" / "learn_queue")
    workspace_manifest_path: Path = field(default_factory=lambda: Path.home() / ".shadou" / "workspace.yaml")
    ...
    # Agent routing
    shadou_route_mode: str = "hybrid"
    shadou_route_agent_max_steps: int = 15
    shadou_route_agent_debug_enabled: bool = False
    shadou_compile_extra_artifacts: bool = False
    ...
    # Integrations
    shadou_github_token: str = ""
    shadou_service_keys_raw: str = ""
    ...
    def resolve_master_faq_path(self) -> Path:
        ...
        try:
            ...
            candidate = self.shadou_home / manifest.paths.knowledge_primary
        ...
        if rag_rel:
            candidate = self.shadou_home / rag_rel
            if candidate.is_file():
                return candidate
        for name in ("workspace.yaml", "00_manifest.yaml"):
            yaml_manifest = self.shadou_home / name
        ...
        for rel in ("knowledge/master_faq.md", "master_faq.md"):
            candidate = self.shadou_home / rel
            if candidate.is_file():
                return candidate
...
def _resolve_workspace_paths() -> tuple[Path, Path, Path, Path, Path, Path, str, Path]:
    from shadou.settings.paths import (
        ShadouPaths,
        resolve_env_file,
        resolve_shadou_home,
    ...
    shadou_home = resolve_shadou_home()
    paths = ShadouPaths(shadou_home)
    env_file = resolve_env_file(shadou_home)
    if env_file.is_file():
    ...
    workspace_manifest = (
        ...
        else shadou_home / "00_manifest.yaml"
    )
    ...
    return (
        ...
        learn_queue,
        workspace_manifest,
        resolve_sop_sync_state_path(shadou_home),
        resolve_session_db_path(shadou_home),
        env_file,
    )
def load_settings() -> Settings:
    ...
    return Settings(
        ...
        shadou_llm_provider=_env("SHADOU_LLM_PROVIDER", "deepseek"),
        shadou_llm_model=_env("SHADOU_LLM_MODEL", deepseek_model),
        shadou_llm_base_url=_env("SHADOU_LLM_BASE_URL", deepseek_base),
        shadou_llm_api_key=_env_nonempty("SHADOU_LLM_API_KEY", "DEEPSEEK_API_KEY"),
        shadou_embed_model=_env("SHADOU_EMBED_MODEL", "text-embedding-3-small"),
        shadou_qdrant_enabled=_env_bool("SHADOU_QDRANT_ENABLED"),
        shadou_qdrant_url=_env("SHADOU_QDRANT_URL", "http://127.0.0.1:6333"),
        shadou_qdrant_api_key=_env("SHADOU_QDRANT_API_KEY"),
        shadou_qdrant_collection=_env("SHADOU_QDRANT_COLLECTION", "shadou_support"),
        shadou_reranker_backend=_env("SHADOU_RERANKER_BACKEND", "provider"),
        shadou_guardrails_enabled=_env_bool("SHADOU_GUARDRAILS_ENABLED"),
        shadou_sop_writeback_enabled=_env_bool("SHADOU_SOP_WRITEBACK_ENABLED"),
        shadou_sop_merge_sync_enabled=_env_bool("SHADOU_SOP_MERGE_SYNC_ENABLED"),
        shadou_sop_merge_sync_hour=_env_int("SHADOU_SOP_MERGE_SYNC_HOUR", 8),
        shadou_sop_merge_sync_minute=_env_int("SHADOU_SOP_MERGE_SYNC_MINUTE", 0),
        ...
        github_repo=_env("SHADOU_GITHUB_REPO") or _env("BUKAPILOT_REPO"),
        github_branch=_env("SHADOU_GITHUB_BRANCH") or _env("BUKAPILOT_BRANCH", "main"),
        bukapilot_repo=_env("SHADOU_GITHUB_REPO") or _env("BUKAPILOT_REPO"),
        bukapilot_branch=_env("SHADOU_GITHUB_BRANCH") or _env("BUKAPILOT_BRANCH", "main"),
        ...
        shadou_route_mode=_env("SHADOU_ROUTE_MODE", "hybrid"),
        shadou_route_agent_max_steps=_env_int("SHADOU_ROUTE_AGENT_MAX_STEPS", 15),
        shadou_route_agent_debug_enabled=_env_bool("SHADOU_ROUTE_AGENT_DEBUG_ENABLED"),
        shadou_compile_extra_artifacts=_env_bool("SHADOU_COMPILE_EXTRA_ARTIFACTS"),
        ...
        shadou_github_token=_env("SHADOU_GITHUB_TOKEN"),
        shadou_service_keys_raw=_env("SHADOU_SERVICE_KEYS"),
