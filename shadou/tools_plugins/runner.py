...
from shadou.tools_plugins.contract import normalize_tool_result, validate_plugin_file


def _shadou_repo_root() -> Path:
    return Path(__file__).resolve().parents[2]


def _plugin_subprocess_env() -> dict[str, str]:
    """Ensure tenant plugins can import the `shadou` package."""
    env = os.environ.copy()
    repo = str(_shadou_repo_root())
...
def resolve_plugin_script(plugin_id: str, params: dict[str, Any]) -> Path | None:
    explicit = str(params.get("script") or params.get("path") or os.getenv(f"SHADOU_PLUGIN_{plugin_id.upper()}_PATH", "")).strip()
...
def run_plugin_tool(
    ...
    *,
    ...
    timeout = timeout_sec or int(params.get("timeout_seconds") or os.getenv("SHADOU_PLUGIN_TIMEOUT_SECONDS", "180"))
    python = os.getenv("SHADOU_PYTHON") or sys.executable
