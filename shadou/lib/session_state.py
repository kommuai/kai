...
def init_db():
    ...
    conn.close()
    try:
        from shadou.lib.session_search import ensure_message_index_schema

        ensure_message_index_schema()
    except Exception:
        pass
    try:
        from shadou.lib.learning_events import init_learning_events_table
...
def add_message_to_history(user_id: str, role: str, text: str):
    ...
    save_session(user_id, sess)
    try:
        from shadou.lib.session_search import index_message
