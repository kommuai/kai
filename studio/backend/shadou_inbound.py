"""Process an inbound WhatsApp (or Studio) user message through the Shadou support runtime."""
...
def main() -> int:
    if len(sys.argv) < 4:
        print(json.dumps({"ok": False, "error": "usage: shadou_inbound.py SHADOU_HOME user_id text [payload_json]"}))
        return 1
    ...
    from shadou.lib.session_state import init_db
